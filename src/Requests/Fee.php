...
class Fee extends Request implements RequestInterface
{
    const URL = "/exchange/v1/fee";
    ...
    public function getUrl(): string
    {
        return self::URL . (!empty($this->queryPath) ? '?' . $this->queryPath :
                '?currency=USD&itemTitle=&gameId=a8db&sellingPrice=&offerType=dmarket&operationType=offer');
    }
